-- ============================================================
-- Panela da Roça — Delivery completo (bruto + líquido + taxa)
-- vendas_delivery = líquido (o que realmente cai no repasse)
-- vendas_delivery_bruto = bruto (o que o cliente pagou)
-- taxa % calculada na UI
-- Idempotente.
-- ============================================================

ALTER TABLE fechamento_dia
  ADD COLUMN IF NOT EXISTS vendas_delivery_bruto NUMERIC DEFAULT 0;
...
CREATE OR REPLACE FUNCTION fechar_dia(
  ...
  p_vendas_delivery_bruto NUMERIC DEFAULT 0
) RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT COALESCE(SUM(valor), 0) INTO v_saidas_caixa
  FROM lancamentos
  WHERE data = p_data AND tipo = 'despesa' AND afeta_caixa = true
    AND origem <> 'fechamento' AND deleted_at IS NULL;

  v_esperado_caixa := ROUND((COALESCE(p_fundo_caixa, 0) + COALESCE(p_vendas_dinheiro, 0) - v_saidas_caixa)::numeric, 2);
  ...
  INSERT INTO fechamento_dia (data, vendas_dinheiro, vendas_cartao_credito, vendas_cartao_debito, vendas_pix,
                              vendas_vale_refeicao, vendas_delivery, vendas_delivery_bruto,
  ...
  VALUES (p_data, p_vendas_dinheiro, p_vendas_cartao_credito, p_vendas_cartao_debito, p_vendas_pix,
          COALESCE(p_vendas_vale_refeicao, 0), COALESCE(p_vendas_delivery, 0), COALESCE(p_vendas_delivery_bruto, 0),
  ...
  ON CONFLICT (data) DO UPDATE SET
    ...
    vendas_delivery_bruto = EXCLUDED.vendas_delivery_bruto,
